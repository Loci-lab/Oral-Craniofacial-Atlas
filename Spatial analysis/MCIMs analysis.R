...
data1=data1[which(data1$TACIT=="Fibroblasts"),]
ggplot(data2, aes(x = X, y = Y),fill="grey90") +
  geom_point(size = 0.1)  +
...
compute_density_exact <- function(data, col, x_col, y_col) {
  ...
  if (nrow(data_sub) > 1) {
    density <- kde2d(data_sub[[x_col]], data_sub[[y_col]], h = c(100, 100), n = 500) #c(10,10,200)
...
ggplot(density_list_final, aes(x = density_list_final$avg_grid_x, y = avg_grid_y, color = as.factor(ifelse(density_list_final$CCL2_CCR5>median(density_list_final$CCL2_CCR5),1,0)))) +
  geom_point(size = 0.1)  +
  theme_minimal()

ggplot(data_wide, aes(x = avg_grid_x, y = avg_grid_y, color = as.factor(ifelse(data_wide$CCL2_CCR5>0,1,0)))) +
  geom_point(size = 0.1)  +
  theme_minimal()
...
seurat_object <- FindVariableFeatures(seurat_object, selection.method = "vst", nfeatures = 20)
...
seurat_object <- FindClusters(seurat_object, resolution = 0.05)  # Algorithm 4 is Leiden
...
data_final_sub=data_final[which(data_final$Group=="MSG9"),]

data_final$Group=data1$Group[data_final$cell_id]
data_final$Cluster=as.numeric(data_final$Cluster)
data_final$Group=ifelse(data_final$Group=="MSG8","BuccalMucosa_v1",data_final$Group)
data_final$Group=ifelse(data_final$Group=="MSG1","BuccalMucosa_v2",data_final$Group)
data_final$Group=ifelse(data_final$Group=="MSG9"&data_final$X_cell<74000,"BuccalMucosa_v3",data_final$Group)
data_final$Group=ifelse(data_final$Group=="BuccalMucosa"&data_final$Y_cell<45000,"MSG_v1",data_final$Group)


data_final_sub=data_final
...
data_final_sub=data_final[which(data_final$Cluster=="3"&data_final$IL6_IL6R==1),]
# Correct ggplot with geom_segment
ggplot() +
  geom_segment(data = data_final_sub, 
               aes(x = X_cell, y = Y_cell, xend = X_neighbor, yend = Y_neighbor, 
                   color = as.factor(data_final_sub$IL6_IL6R)),  # Correct reference to Cluster
               na.rm = TRUE, size = 1) +
  theme_classic(base_size = 15) +
  scale_color_manual(values = user_colors) +  # Ensure user_colors matches the factors in Cluster
  labs(x = "X", y = "Y", title = "") +
...
table(data_final_sub$GroupNICHES)




# Create a table with the counts
table_counts <- table(data_final$Cluster, data_final$IL6_IL6R)

# Extract the counts where IL6_IL6R = 1 (second column)
il6_il6r_1 <- table_counts[, 2]

# Calculate the total number of occurrences in each cluster (sum of both 0 and 1 counts)
total_per_cluster <- rowSums(table_counts)

# Calculate enrichment by dividing occurrences where IL6_IL6R = 1 by the total for each cluster
enrichment <- il6_il6r_1 / total_per_cluster

# Display the enrichment values for each cluster
enrichment
...
data_final$GroupNICHES=ifelse(data_final$Group%in%c("Gingiva1","Gingiva2"),"Gingiva",data_final$Group)
data_final$GroupNICHES=ifelse(data_final$GroupNICHES%in%c("MSG1","MSG8","MSG9","MSG_v1"),"MSG",data_final$GroupNICHES)
...
data_final$GroupNICHES=ifelse(data_final$GroupNICHES%in%c("BuccalMucosa_v2","BuccalMucosa_v1","BuccalMucosa_v3"),"BuccalMucosa",data_final$GroupNICHES)

data_final$GroupMG=ifelse(data_final$GroupNICHES%in%c("BuccalMucosa","Gingiva","Tongue"),"Mucosal","Glands")
...
for (neighbor in neighbors) {
  ...
  ggsave(paste0("C:/Users/huynhk4/Downloads/MERSCOPE_LR_2309/Mucosal vs Glands/Spatial/", neighbor, "_LR_spatial.svg"), plot = p, width = 10, height = 8, dpi = 300)
}
...
group_colors <- c(
  "BuccalMucosa" = "#FF9999", 
  "BuccalMucosa_v1" = "#FFA07A", 
  "BuccalMucosa_v2" = "#FF6347", 
  "BuccalMucosa_v3" = "#FF4500",
  "Gingiva1" = "blue", 
  "Gingiva2" = "#99FF99",
  "MSG_v1" = "#FFCC99", 
  "MSG9" = "#FF66B2",
...
data_plot$Group2=ifelse(data_plot$Group%in%c("MSG1","MSG_v1","MSG9"),"MSG",data_plot$Group2)
...
data_plot$Group2=ifelse(data_plot$Group%in%c("BuccalMucosa","BuccalMucosa_v1","BuccalMucosa_v2","BuccalMucosa_v3"),"BuccalMucosa",data_plot$Group2)
...
data_plot=data.frame(Group=data_final$GroupMG,Cluster=data_final$Cluster)
...
data_plot_prop <- data_plot_prop %>%
  group_by(Cluster) %>%
  mutate(mucosal_proportion = sum(proportion[Group == "Mucosal"])) %>%
  ungroup() %>%
...
ggplot(data_plot_prop, aes(x = Cluster, y = proportion, fill = Group)) +
  geom_bar(stat = "identity", position = "stack") +
  scale_fill_manual(values = group_colors) +
...
filtered_data=filtered_data[!(filtered_data$Group%in%c("BuccalMucosa_v2","Gingiva2","Tongue12","Tongue62")),]
...
for (group_name in groups) {
  ...
  filename <- paste0("C:/Users/huynhk4/Downloads/MERSCOPE_LR_2309/Mucosal vs Glands/Motif/Group/OCF_motif_", group_name, ".svg")
  svg(filename)
  plot(graph,
       edge.width = E(graph)$weight * 10,  # Thickness of the edges proportional to the connection strength
       vertex.size = 15,                   # Size of the vertices
...
top5 = scfp.markers %>%               # Filter markers with avg_log2FC > 1
  group_by(cluster) %>%                     # Group by cluster (or group)
  slice_max(order_by = avg_log2FC, n = 5) %>% # Select top 5 markers for each cluster
...
library(tidyr)
library(tibble)
# Define top ligand-receptor pairs for each cluster based on the top5 data
lr_dict <- top5 %>%
  group_by(cluster) %>%
  summarise(lr_pairs = list(unique(gene))) %>%
  deframe()

# Loop through each cluster and create plots
for (cluster in names(lr_dict)) {
  # Subset the data for the current cluster and select only relevant ligand-receptor pairs
  selected_lr <- lr_dict[[cluster]]
  
  # Filter the data where the cluster matches and any of the selected ligand-receptor interactions are 1
  data_filtered <- data_final %>%
    filter(Cluster == cluster) %>%
    filter_at(vars(one_of(selected_lr)), any_vars(. == 1))
  
  # Pivot the data longer for easier coloring by ligand-receptor interaction
  data_long <- data_filtered %>%
    pivot_longer(cols = one_of(selected_lr), names_to = "Ligand_Receptor", values_to = "Value") %>%
    filter(Value == 1)  # Keep only rows where the value is 1
  
  # Check if there's data to plot
  if (nrow(data_long) > 0) {
    # Create the plot
    p <- ggplot(data_long, aes(x = X_cell, y = Y_cell, color = Ligand_Receptor)) +
      geom_segment(aes(xend = X_neighbor, yend = Y_neighbor), size = 1, na.rm = TRUE) +
      theme_classic() +
      facet_wrap(~ Group, scales = "free") +  # Facet by Group
      labs(x = "X Coordinate", y = "Y Coordinate", 
           title = paste("Ligand-Receptor Interactions in Cluster", cluster)) +
      theme(legend.title = element_text(size = 10), legend.text = element_text(size = 8)) +
      guides(color = guide_legend(title = "Ligand-Receptor Interactions"))
    
    # Save the plot as an SVG
    ggsave(filename = paste0("C:/Users/huynhk4/Downloads/MERSCOPE_LR_2309/Mucosal vs Glands/Spatial co-loccation LR pair/Cluster_", cluster, "_signature.svg"), plot = p, device = "svg", width = 15, height = 12)
  }
}
...
write.csv(as.data.frame(top5),"C:/Users/huynhk4/Downloads/MERSCOPE_LR_2309/Mucosal vs Glands/LR_signature_per_cluster.csv")
...
data_final_sub=data_final[which(data_final$GroupNICHES%in%c("BuccalMucosa","Gingiva","Tongue")),]

library(Seurat)
data = data_final_sub[,colnames(count)]
orig_values = as.matrix(data)  # Consider only marker data
rownames(orig_values) = 1:nrow(data)
orig_values = t(orig_values)
orig_values_metadata = data.frame("CellID" = 1:nrow(data))
...
Idents(scfp)=ifelse(data_final_sub$GroupNICHES%in%c("BuccalMucosa"),"BuccalMucosa","Gingiva & Tongue")


# Run differential expression analysis
markers <- FindMarkers(scfp, ident.1 = "BuccalMucosa", ident.2 = "Gingiva & Tongue", test.use = "wilcox")
...
markers$significance <- with(markers, ifelse(p_val_adj < 0.05 & abs(avg_log2FC) > 0.25, 
                                             ifelse(avg_log2FC > 0.25, "Up in BuccalMucosa", "Up in Gingiva & Tongue"), 
                                             "Not Significant"))
...
ggplot(markers, aes(x = avg_log2FC, y = -log10(p_val_adj_plot), color = significance)) +
  ...
  scale_color_manual(values = c("Up in BuccalMucosa" = "red", "Up in Up in Gingiva & Tongue" = "blue", "Not Significant" = "gray")) +
...
data_plot=data.frame(TACIT_Ligands=data_final$TACIT_Ligands,TACIT_Receptors=data_final$TACIT_Receptors,count,Group=data_final$Group)
...
data_plot=data_plot[which(data_plot$Group%in%c("BuccalMucosa")),]
data_plot=data_plot[,-54]
# Function to calculate top interactions for each cell type
get_top_interactions <- function(cell_type, data_plot, top_n = 50) {
...
desired_columns <- c("Fibroblasts", "LECs", "Mural Cells", "VECs")

# Find the missing columns
missing_columns <- setdiff(desired_columns, colnames(heatmap_matrix))

# If there are any missing columns, create them with zero values
if(length(missing_columns) > 0) {
  # Create a matrix of zeros with the same number of rows as heatmap_matrix and columns for the missing columns
  zero_matrix <- matrix(0, nrow = nrow(heatmap_matrix), ncol = length(missing_columns))
  colnames(zero_matrix) <- missing_columns
  
  # Bind the zero_matrix to the original heatmap_matrix
  heatmap_matrix <- cbind(heatmap_matrix, zero_matrix)
}

# Now subset the matrix with the desired columns
heatmap_matrix <- heatmap_matrix[, desired_columns]
...
pheatmap(heatmap_matrix[which(rowSums(heatmap_matrix) > 0), ], 
         ...
         fontsize_row = 15,     # Font size for Y-axis (row labels)
         fontsize_col = 15)     # Font size for X-axis (column labels)
